...
$wheretest = "where 1 order by test_id DESC";
$wherelocation = "where 1";
$wherestandard = "where 1";

if ($_SESSION['usertype'] == "Parent") {
    $wheretest = "where branch= $location order by test_id DESC";
    $wherelocation = "where location_id = $location";
    $wherestandard = "where 1";
}
$findlocation = find("all", $table2, "*", $wherelocation, []);
$findstandard = find("all", $table3, "*", $wherestandard, []);

if (isset($_POST["del"])) {
    $test_id1 = $_POST["testid"];
    $where_clause = " where test_id ='$test_id1' ";
    $execute = [];
...
curl_setopt_array($curl, array(
  CURLOPT_URL => 'https://api.zoom.us/v2/users?type=live&page_size=30&page_number=1',
  ...
  CURLOPT_HTTPHEADER => array(
    'Authorization: Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOm51bGwsImlzcyI6Ik9yY0tndkt2Uzc2bTdrN1pOLWh5UEEiLCJleHAiOjE5NTY0ODY2MDAsImlhdCI6MTYzNjk2MTgxNX0.0DFX2xvNB4Qyy6YEbLkI0O4QM0gVkZYjtqiuPY4N3V0',
    'Cookie: _zm_csp_script_nonce=l5tGG45vRmKhO_tFqFsamw; _zm_currency=INR; _zm_mtk_guid=cff2572e27f84c6a8f9a5b6711493111; _zm_ssid=aw1_c_Tf6uQJfkRbCDHylCX7umsg; cred=23F9BFC51B52AB10D6279134443115F4'
  ),
));
...
curl_close($curl);
// echo $response1; echo"<br>";



$result1 = json_decode(json_encode(json_decode($response1), true),true);
// print_r($result1);
...
curl_setopt_array($curl, array(
  CURLOPT_URL => 'https://api.zoom.us/v2/report/meetings/85109726468/participants',
  ...
  CURLOPT_HTTPHEADER => array(
    'Authorization: Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOm51bGwsImlzcyI6Ik9yY0tndkt2Uzc2bTdrN1pOLWh5UEEiLCJleHAiOjE5NTY0ODY2MDAsImlhdCI6MTYzNjk2MTgxNX0.0DFX2xvNB4Qyy6YEbLkI0O4QM0gVkZYjtqiuPY4N3V0',
    'Cookie: _zm_mtk_guid=cff2572e27f84c6a8f9a5b6711493111; cred=8BFB0A55D88451E0F6BD7F3202D4D8CE'
  ),
));
...
foreach($result1[$keys1[$j]] as $key => $vall){
...
foreach($result[$keys[$i]] as $key => $val){
...
// $fields= "id,user_id,meeting_id,user_email,name,join_time,leave_time,duration,attentiveness_score,failover,customer_key";
// $values= ":id,:user_id,:meeting_id,:user_email,:name,:join_time,:leave_time,:duration,:attentiveness_score,:failover,:customer_key";
// $exe= array(":id"=>$id,":user_id"=>$user_id,":meeting_id"=>$pmi,":user_email"=>$user_email,":name"=>$name,":join_time"=>$join_time,":leave_time"=>$leave_time,":duration"=>$duration,":attentiveness_score"=>$attentiveness_score,":failover"=>$failover,":customer_key"=>$customer_key); 


//  $save= save("attendinfo", $fields, $values, $exe);
    ...
    ***********************************-->
    <div id="main-wrapper">
        ...
        ***********************************-->
        <div class="nav-header">
            <a href="index.html" class="brand-logo">
                ...
                <img class="brand-title" src="Shobhaslogowebsite.png" alt="">
            </a>
            ...
            <div class="container-fluid">
                <div class="page-titles">
					<ol class="breadcrumb">
						...
						<li class="breadcrumb-item active"><a href="javascript:void(0)">Add Test</a></li>
					</ol>
                </div>
                <!-- row -->
                <div class="row">
                    <!-- <div class="card-header mx-0 my-0 px-0 py-0">
                        <div class="row" style="background-color:#FFD500; color:#2C004E; border-radius:30px 30px 0px 0px;padding: 10px 0 2px 0;">
                            <center>
					            <h5><?php if (($_SESSION["roll"] == '4')||($_SESSION["roll"] == '1')) { ?> Add <?php }else{ ?>View <?php }?> Test</h5>
                            </center>
                        </div>
                    </div> -->

                    <form action="ajax/savetestinfo.php" id="addtest" method="POST">
              <div class="card-body Px-3">
                <!-- Row 1 close -->
				<?php if (($_SESSION["roll"] == '4')||($_SESSION["roll"] == '1')) { ?>
				 <div class="row">
					<div class="col-sm-3">
						<select class="form-control " name="location_id" id="location_id" onchange="locationchange(this.val)">
						  <option>All Branches</option>
						  <?php foreach ($findlocation as $key => $val) { ?>
						  <option value="<?php echo $val["location_id"]; ?>"><?php echo $val["location_name"]; ?></option>
						  <?php } ?>
						</select>
					</div>

					<div class="col-sm-3 locationdetails">
						<select class="form-control " name="class_id" id="class_id">
						  <option value="">Select Class</option>
						</select>
					</div>
					
					<div class="classdetails col-sm-6 row" >
					  <div class="col-sm-6">
						<select class="form-control " name="batch_id" onchange="batchtocourse(this.value)" id="batch_id">
					...
					<div class="col-md-12">
				<div class="row mt-2 fetchfromglobaldb">
				</div>
				<div class="row mt-2 fetchfromglobaldbsubject">
				</div>
			</div>
				 </div>
				   <br>
                <div class="row">
				  ...
				  <lable>Select Date & Time</lable>
                    <input type="datetime-local" name="datetime" placeholder="Select Date & Time*" class="form-control" id="datetime" required>
                  </div>
				  
				   <div class="col-sm-3">
				   <lable>End Time</lable>
					<input type="time" name="test_time" placeholder="Enter Time*" class="form-control">
                  </div>
                  ...
                  <!--<div class="col-sm-3">
					...
					<div class="col-sm-3" style="margin-top: 24px;">
                    <input type="text" class="form-control" name="syllabus" placeholder="Enter Syllabus">
                  </div>
                ...
                <div class="row">
					<!-- <div class="col-sm-3">
						<select name="class" class="form-select">
						  ...
						  <?php foreach ($findstandard as $key => $val) { ?>
						  <option value="<?php echo $val["std_id"]; ?>"><?php echo $val["class"]; ?></option>
						  <?php } ?>
						</select>
					  </div>
				  
					<div class="col-sm-3">
						<input type="text" class="form-control" name="batch" placeholder="Enter Batch Name*" required>
				...
				<div class="row" style="margin-top:15px;">  
				 <div class="col-sm-4">
                    ...
                    <!--<a class="btn btn-light btn-dark down-icon rounded-pill" name="fileToUpload" id="fileToUpload" href="#"><i class="fa fa-paperclip fa-lg"></i></a>-->
                  </div>
                  ...
                  <div class="col-sm-3">
                    <a class="btn btn-dark down-icon-n rounded-pill btn-block" style="width:200px; height:40px; margin-top:23px; display: flex;align-content: space-around;flex-direction: column;justify-content: space-around;"
                      href="javascript:void(0)" onclick="addtest()">Add Test</a>
                  </div>
                </div>
				 <?php } ?>
                <!-- Row 2 Close -->
                ...
                <div class="row  py-2">
                      ...
                      <div class="col-sm-3">
                        <a class="btn btn-dark down-icon-n rounded-pill btn-block" style="width:200px; height:40px; display: flex;align-content: space-around;flex-direction: column;justify-content: space-around;"
                          href="#" onclick="sendData()">Find</a>
                      </div>
                </div>
                <!-- Row Table start-->
                <br>
                <div class="row">
                      <table id="testinfo_Table" class="table table-bordered text-center table-hover" style="border-top: 1px solid #111; width:125%">
                            <thead class="subtitle" style="white-space: nowrap;">
        ...
        ***********************************-->
		<div class="footer">
            <div class="copyright">
                <p>Copyright © Designed &amp; Developed by <a href="https://tdtl.world" target="_blank">The Data Tech Lab</a> 2022</p>
            </div>
        </div>
        <!--**********************************
            Footer end
  ...
  <script src="https://cdnjs.cloudflare.com/ajax/libs/sweetalert/2.1.2/sweetalert.min.js"></script>
   ...
   <script>
    $(function(){
		...
		$('#testinfo_Table').DataTable( {
				"processing": true,
				"serverSide": true,
				"ajax": { url : "ajax/addtest.php" , type : 'POST'}
			} );

	  });
 
	    function addtest(){
			$('#addtest').ajaxForm(function(data){
		...
		function sendData() {
			 ...
			 $.ajax({
				method: "POST",
				url: "ajax/filter.php",
				data: { std: standard , loc: location }
			 }).done(function(response) {
				$("#findtest").html(response);
			 });
		  }
        ...
        var location_id = $("#location_id").val();
		 ...
		 $.ajax({
			  method: "POST",
			  url: "ajax/locationclassphp.php",
			  data: { getdatatype: "selectbox", location_id: location_id }
			})
			  .done(function( msg ) {
				$(".locationdetails").html(msg);
			  });
      }
	   function classtosubject(class_id)
	  {
      var class_id = $("#class_id").val();
          ...
          method: "POST",
			  url: "ajax/locationclassphp.php",
			  data: { getdatatype: "selectbox", class_id: class_id }
			})
			  .done(function( msg ) {
				$(".classdetails").html(msg);
			  });
	  }

	  function batchtocourse(batch_id)
	  {
		   $.ajax({
          method: "POST",
			  url: "globaldb/coursetosubjectajax.php",
			  data: { getdatatype: "selectbox", batch_id: batch_id }
			})
			  .done(function( msg ) {
				$(".fetchfromglobaldb").html(msg);
			  });
	  }


	  function coursetosubject(course_id)
	  {
		var class_id= $("#class_id").val();
		var br_id= $("#location_id").val();
		   $.ajax({
          method: "POST",
			  url: "globaldb/coursetosubjectajax.php",
			  data: { getdatatype: "selectbox", course_id: course_id, class_id:class_id, br_id:br_id }
			})
			  .done(function( msg ) {
				$(".fetchfromglobaldbsubject").html(msg);
			  });
	  }
